...
Data: radiocarbon_incubations_Barrow_2012_2014.csv includes CO2 flux rates and radiocarbon values from incubated soils.  File and metadata record can be accessed at http://dx.doi.org/10.5440/1364062
```{r}
...
BulkPlotb + plottheme
ggsave("BulkPlotb.png", path="plots", width = 6,height = 6,dpi = 300)
```
...
PoolsPlot_20_10000 <- ggplot(PoolsDelta_20_10000, aes(colour=pool, shape=pool, name="Carbon pool", y=as.numeric(Delta2), x=as.numeric(Delta3))) +
  ...
  ylab(expression(Delta[2]~"(%)")) +
  xlab(expression(Delta[3]~"(%)")) +
  scale_colour_manual(values=c("#4074DC","#FCAA3D"), name="Carbon pool", labels=c("Active (TT = 20 y)", "Passive (TT = 10,000 y)")) +
...
```{r}
legendthemeb <- theme(legend.title = element_text(size=14)) +
  theme(legend.text = element_text(size=12)) +
...
```{r}
hist(PoolsDelta_50_5000$Delta2b)
qqPlot(PoolsDelta_50_5000$Delta2b, "norm")
...
#Validate model
plot(fitted(PoolsModelb_50_5000_reduced),residuals(PoolsModelb_50_5000_reduced))
hist(residuals(PoolsModelb_50_5000_reduced))
...
Do the same with Delta2b and Delta3
```{r}
hist(PoolsDelta_20_10000$Delta2b)
qqPlot(PoolsDelta_20_10000$Delta2b, "norm")
...
#Validate model
plot(fitted(PoolsModelb_20_10000_reduced),residuals(PoolsModelb_20_10000_reduced))
hist(residuals(PoolsModelb_20_10000_reduced))
...
print(PoolsPlotb_50_5000 + plottheme + legendthemeb)
ggsave("PoolsPlotb_50_5000.png", path="plots", width = 6.235,height = 6.13, dpi = 1200)
...
print(PoolsPlotb_20_10000 + plottheme + legendthemeb)

ggsave("PoolsPlotb_20_10000.png", path="plots", width = 6.235,height = 6.13,dpi = 1200)
```
...
p3 <- BulkPlotb + plottheme
p4 <- PoolsPlotb_50_5000 + plottheme + legendthemeb
combob <- grid.arrange(p3, p4, nrow=2, heights=c(6,6))
ggsave("CombinedPlotb.png", combob, path="plots", width = 6,height = 11,dpi = 1200)
```